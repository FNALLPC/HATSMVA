...
def plotData():
    ...
    cmass = TCanvas("fig_%s_VV_gg" % CWD, "VBF/ggF",
                    10, 10, 500, 500)    
...
def main():
    ...
    bestcuts = {}
    for name in ['f_deltajj', 'f_massjj']:    
        value = ntuple.get(name)
        bestcuts[name] = value
        print "\t%s" % name
        for ii in xrange(len(value)):
            print "\t\t%10.2f" % value[ii]
    print
    ...
    hcut.AddBin(bestcuts['f_deltajj'][0], bestcuts['f_massjj'][0],
                bestcuts['f_deltajj'][1], bestcuts['f_massjj'][1])
    cmass.cd()
    hs.Draw('p')
    hb.Draw('p same')
    hcut.Draw('same')
